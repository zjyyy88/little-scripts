...
API key 优先从环境变量 MP_API_KEY 读取，也可以用 --api-key 临时传入。
建议在 PowerShell 中永久配置：
  [Environment]::SetEnvironmentVariable("MP_API_KEY", "你的key", "User")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--api-key",
        help="MP API key；省略时读取环境变量 MP_API_KEY。",
    )
    parser.add_argument(
        "--cache",
